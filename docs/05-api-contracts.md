...
| `GET` | `/chat/sessions/:anonymousPublicId/messages` | Lista mensajes. |
| `POST` | `/chat/sessions/:anonymousPublicId/messages` | Añade mensaje. Cuerpo: `content`, `role`, `authorDoctorUserId?` (obligatorio si `role === user`). Si `role === user`, el backend orquesta la llamada al Brain y persiste la respuesta del asistente. |
...
| `GET` | `/documents?kind=&patientId=` | Listado filtrado. |
