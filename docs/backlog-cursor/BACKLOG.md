...
| **INF-2** | Variable `BRAIN_SERVICE_URL` consumida por el cliente HTTP del Back hacia el Cerebro.             | RagService (o equivalente) lee URL desde config; fallo claro si Cerebro no alcanzable en entorno dev.       | **Pendiente** | [docker-compose.yml](../../docker-compose.yml) |
...
| **BK-11** | Registrar en **audit log** interacciones críticas (consulta RAG, cierre chat, upload) sin depender solo de `POST /audit/logs` manual. | Eventos automáticos con `patientId` / `clinicalHistoryId` / metadata útil.                               | **Pendiente** | [backend/src/modules/audit/](../../backend/src/modules/audit/) |
...
| **BK-13** | **RagService**: cliente HTTP hacia Pinky (`POST /query` mínimo); parsing de `answer` y `sources`.   | Módulo RAG deja de estar vacío; errores de red/timeout tratados.                                           | **Pendiente** | [backend/src/modules/rag/rag.service.ts](../../backend/src/modules/rag/rag.service.ts) |
