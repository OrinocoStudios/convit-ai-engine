...
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------------------------------------ |
| **ID-1** | Definir si DNI/NSS es el `patientId` expuesto al Cerebro o un campo (`externalPatientId`) con mapeo al `_id` Mongo; unicidad por `tenantId`.                     | Claridad en la API y el esquema de Mongo; Pinky usa ID interno; búsqueda por documento en Back.                 | **Hecho**   | [docs/adr/0001-patient-identifier-dni-ssn.md](../adr/0001-patient-identifier-dni-ssn.md), [backend/src/modules/patients/](../../backend/src/modules/patients/) |
...
| **INF-3** | (Opcional) Servicio Pinky en Compose o documentación de despliegue conjunto.                      | `docs/08-deployment.md` describe compose mínimo (Back+Mongo) y cómo enlazar Pinky/Ollama vía `BRAIN_SERVICE_URL`.                            | **Hecho**   | [docs/08-deployment.md](../08-deployment.md), [README.md](../../README.md)   |
| **INF-4** | Healthchecks y/o readiness para backend y dependencias.                                           | `GET /health` (liveness) y `GET /health/ready` (Mongo); documentado en `docs/05-api-contracts.md` y `docs/08-deployment.md`.                                                            | **Hecho**   | [backend/src/health/](../../backend/src/health/), [docs/getting-started.md](../getting-started.md) |
...
| **BK-2**  | Modelo paciente con identificador clínico DNI o NSS (según **ID-1**).                               | Campo(s) validados; unicidad por tenant; búsqueda/lookup para iniciar chat; al menos uno obligatorio.      | **Hecho**   | [backend/src/modules/patients/](../../backend/src/modules/patients/) |
...
| **BK-4**  | Almacenamiento de ficheros (filesystem o S3-compatible) y `storageKey` real.                        | Upload descargable; política de tamaño para tool calls según diseño.                                       | **Hecho**   | [backend/src/modules/storage/](../../backend/src/modules/storage/) |
...
| **BK-6**  | **ChatSummary** en almacén separado (o colección dedicada) y endpoints de listado/creación.       | Vinculado a `clinicalHistoryId`; usable como contexto en respuestas con fuente `CHAT_SUMMARY` si aplica.   | **Hecho**   | [backend/src/modules/chat-summaries/](../../backend/src/modules/chat-summaries/) |
...
| **BK-10** | Flujo **fin de chat**: generar resumen de la conversación; enviar al Cerebro como documento del paciente; **reindexar** paciente existente o **crear** índice nuevo. | Documentado el orden de llamadas a Pinky; idempotencia o manejo de errores básico.                       | **Hecho**   | [backend/src/modules/chat/chat.service.ts](../../backend/src/modules/chat/chat.service.ts) |
